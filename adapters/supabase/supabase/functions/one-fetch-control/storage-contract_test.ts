...
Deno.test("Malformed login and refresh RPC results fail as storage drift", async () => {
  ...
  const login = await createControlHandler(environment, loginDatabase)(
  ...
  const refresh = await createControlHandler(environment, refreshDatabase)(
...
Deno.test("Malformed revoke results cannot become successful responses", async () => {
  const sessionDatabase: Database = {
    rpc: <T>(name: string) => {
      if (name === "of_authenticate_access") return Promise.resolve(admin as T);
      if (name === "of_revoke_session") return Promise.resolve("false" as T);
      return Promise.reject(new Error(`unexpected RPC ${name}`));
    },
  };
  const logout = await createControlHandler(environment, sessionDatabase)(
  ...
  const revoked = await createControlHandler(environment, tokenDatabase)(
...
Deno.test("Malformed list and report records fail at the storage boundary", async () => {
  const sessionsDatabase: Database = {
    rpc: <T>(name: string) => {
      if (name === "of_authenticate_access") return Promise.resolve(admin as T);
      if (name === "of_list_sessions") {
        return Promise.resolve(
          [{ schemaVersion: 1, id: "missing-fields" }] as T,
        );
      }
      return Promise.reject(new Error(`unexpected RPC ${name}`));
    },
  };
  const sessions = await createControlHandler(environment, sessionsDatabase)(
  ...
  const report = await createControlHandler(environment, reportDatabase)(
...
Deno.test("Health and configuration reject a different stored instance", async () => {
  ...
  const health = await createControlHandler(environment, healthDatabase)(
...
Deno.test("Database transport, timeout, and invalid JSON become 503 errors", async () => {
  ...
  await expectControlError(
    await createControlHandler(environment, transport)(
      request("/api/v1/health"),
    ),
    503,
    "storage_unavailable",
