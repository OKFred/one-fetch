...
Deno.test(
  "Every Control route checks migration compatibility first",
  async () => {
...
Deno.test(
  "Control distinguishes migration mismatch, contract, and storage failures",
  async () => {
...
Deno.test(
  "A successful Control migration check is briefly shared",
  async () => {
    let checks = 0;
    const database: Database = {
      rpc: <T>(name: string) => {
