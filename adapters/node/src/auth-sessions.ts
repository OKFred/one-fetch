...
export class AdminSessionService {
  ...
  async refresh(refreshToken: string): Promise<IssuedSession> {
    ...
    if (token.revoked_at || token.used_at)
      await this.rejectRefreshReuse(token.family_id);
    const now = new Date().toISOString();
    if (token.expires_at <= now) throw new Error("Refresh token expired");
      ...
      await this.database.transaction([
        {
          expectedChanges: 1,
          kind: "run",
          sql: `UPDATE auth_tokens SET used_at = ?, revoked_at = ?
                WHERE id = ? AND used_at IS NULL AND revoked_at IS NULL
                  AND expires_at > ?`,
          parameters: [now, now, token.id, now],
        },
        ...issued.operations,
        this.audit.prepare({
  ...
  private async rejectRefreshReuse(familyId: string | null): Promise<never> {
    if (familyId) {
